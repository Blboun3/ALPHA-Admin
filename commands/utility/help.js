module.exports = {
    name: 'help', // Jméno
    description: 'Vypíše seznam všech příkazů nebo podrobné informace o jednom příkazu', // Popis
    aliases: ['commands', 'hlp'], // Aliasi
    usage: '[command name]', // použití
    cooldown: 5, // Cooldown
    execute(message, args) { // Spuštění
        const data = []; // Data [] prázndé pole
        const {
            commands
        } = message.client; // Not sure about that

        if (!args.length) { // pokud nejsou argumenty 
            if(message.channel.type === 'dm'){
                data.push('Tady je seznam všech příkazů:');
                data.push(commands.map(command => command.name).join(', ')); // ? IDK
            }else {
                data.push(`Tady je seznam všech příkazů, které můžeš používat na serveru ${message.guild.name}:`);
        ...
        if (command.usage) data.push(`**Použití:** ${__prefix}${command.name} ${command.usage}`);

        data.push(`**Cooldown:** ${command.cooldown || 3} vteřin`);

        message.channel.send(data, { // Napsání
            split: true
        });
    },
};